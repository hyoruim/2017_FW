...
mod_data_n = mod_data+normrnd(0, 4, 1,length(mod_data)); % AWGN noise when sigma is 4
result_with_noise4=demod(mod_data_n);
%for BER gragh

Pb=zeros(1,10);
for k=1:10
mod_data_n = mod_data+normrnd(0, k, 1,length(mod_data)); % AWGN noise 
demod_n=demod(mod_data_n); %result with noise using function
Pb(k)=sum(sum(xor(result_without_noise,demod_n)))/10000;
end
l=(1:10);
semilogy(log10(1./l),Pb,'r*-'); grid;
title('BER Gragh'); xlabel('log(1/\sigma^2)'); ylabel('Pb');
